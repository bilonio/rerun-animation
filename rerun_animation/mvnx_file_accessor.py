...
class MvnxFileAccessor:
    ...
    def get_ergo_joint_angle_xzy(self, joint, frame=mvn.FRAMES_ALL, angle=mvn.ANGLE_ALL):
        joint_name = mvn.ERGO_JOINTS[joint]
        data_set = 'ergo_joint_data_xzy'
    ...
    def get_joint_connections(self, joints):
        # Extract only 'elements' from joints data
        joint_elements = joints['elements']
        
        # List to store ONLY the segment indexes (e.g., [[0, 1], [1, 2], ...])
        all_segment_indices = []
        
        # Iterate through the dictionaries
        for joint_element in joint_elements:
            # Get the seg_points array for this joint (e.g., array([[0, 1], [1, 0]]))
            seg_point_array = joint_element['seg_points']
            
            # Use NumPy slicing: [:, 0] selects all rows, first column
            segment_indices_pair = seg_point_array[:, 0] 
            # Result for the example above would be: array([0, 1])

            all_segment_indices.append(segment_indices_pair)
            
        return all_segment_indices
    ...
    def get_sensor_magnetic_field(self, segment, frame=mvn.AXIS_ALL, axis=mvn.AXIS_ALL):
        """ 
        Get the magnetic field information for a sensor 
        ...
        return self.get_sensor_data('mag', segment, frame, axis)
    ...
    def get_data(self, data_set, data_field, segment, frame=mvn.FRAMES_ALL, axis=mvn.AXIS_ALL):
        if segment == mvn.SEGMENT_CENTER_OF_MASS:
            segment_name = 'com'
        else:
            segment_name = mvn.SEGMENTS[segment]
